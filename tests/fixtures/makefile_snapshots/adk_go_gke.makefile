...
deploy:
	...
	gcloud container clusters get-credentials test-go-agent-dev --region us-east1 --project $$PROJECT_ID && \
	IMAGE_TAG=$${IMAGE_TAG:-$$(date +%Y%m%d%H%M%S)} && \
	IMAGE=us-east1-docker.pkg.dev/$$PROJECT_ID/test-go-agent/test-go-agent:$$IMAGE_TAG && \
